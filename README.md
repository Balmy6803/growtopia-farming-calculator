...
* **Apple Strudel:** 5% tree growth reduction.
...
* **Cosmic Cape:** 0.1% (1 in 1,000) Comet Dust drop rate per block destroyed.
...
* **Conservative Floor Rounding:** All primary KPI metrics—including Total Profit (WLs), Net Seed Gain, Total Gems Farmed, and Comet Dust WL value—are strictly rounded down (`Math.floor`) to whole integers to provide conservative baseline projections.
...
Base growth time in seconds ($T$) as a function of item rarity ($R$):

$$T = R \times (R^2 + 30)$$

When the Green Ancestor artifact ($k_{\text{green}} \in [0.02, 0.12]$) and/or Apple Strudel ($k_{\text{strudel}} = 0.05$) are active, growth reduction applies multiplicatively:

$$T_{\text{effective}} = T \times (1 - k_{\text{green}}) \times (1 - k_{\text{strudel}})$$
...
*Note: The code used for this calculator was completely written, designed, and coded by Google Gemini.*# Growtopia Farming Calculator
